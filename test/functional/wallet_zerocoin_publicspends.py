...
class ZerocoinSpendTest(BltgTestFramework):
    ...
    def run_test(self):
        ...
        def check_balances(denom, zbltg_bal, bltg_bal):
            zbltg_bal -= denom
            assert_equal(self.nodes[2].getzerocoinbalance()['Total'], zbltg_bal)
            bltg_bal += denom
            wi = self.nodes[2].getwalletinfo()
            assert_equal(wi['balance'] + wi['immature_balance'], bltg_bal)
            return zbltg_bal, bltg_bal
        ...
        zbltg_balance = self.nodes[2].getzerocoinbalance()['Total']
        assert_equal(balance, DecimalAmt(13833.92))
        assert_equal(zbltg_balance, 6666)
        ...
        zbltg_balance, balance = check_balances(denom_2, zbltg_balance, balance)
        ...
        block_time = stake_4_blocks(block_time)
        self.check_tx_in_chain(0, txid)
        zbltg_balance, balance = check_balances(denom_3, zbltg_balance, balance)
        ...
        _, _ = check_balances(denom_1, zbltg_balance, balance)
        self.log.info("--> VALID PUBLIC COIN SPEND (v3) PASSED")

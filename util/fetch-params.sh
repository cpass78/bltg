...
if [ -n "${1:-}" ]; then
    ...
    if [[ "$OSTYPE" == "darwin"* ]]; then
        PARAMS_DIR="$HOME/Library/Application Support/BLTGParams"
    else
        PARAMS_DIR="$HOME/.bltg-params"
    fi
fi
...
Failed to fetch the BLTG zkSNARK parameters!
...
function main() {
    ...
    cat <<EOF
BLTG - fetch-params.sh

This script will fetch the BLTG zkSNARK parameters and verify their
...
EOF
    ...
    then
        ...
        cat >> "$README_PATH" <<EOF
This directory stores common BLTG zkSNARK parameters. Note that it is

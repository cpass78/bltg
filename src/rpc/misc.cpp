...
UniValue getinfo(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() != 0)
        throw std::runtime_error(
            ...
            "  \"balance\": xxxxxxx,           (numeric) the total bltg balance of the wallet (excluding zerocoins)\n"
            ...
            "  \"paytxfee\": x.xxxx,           (numeric) the transaction fee set in bltg/kb\n"
            "  \"relayfee\": x.xxxx,           (numeric) minimum relay fee for non-free transactions in bltg/kb\n"
    ...
    for (auto denom : libzerocoin::zerocoinDenomList) {
        if (mapZerocoinSupply.empty())
            zbltgObj.push_back(Pair(std::to_string(denom), ValueFromAmount(0)));
        else
            zbltgObj.push_back(Pair(std::to_string(denom), ValueFromAmount(mapZerocoinSupply.at(denom) * (denom*COIN))));
    }
    zbltgObj.push_back(Pair("total", ValueFromAmount(GetZerocoinSupply())));
    obj.push_back(Pair("zBLTGsupply", zbltgObj));
...
UniValue validateaddress(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() != 1)
        throw std::runtime_error(
            "validateaddress \"bltgaddress\"\n"
            "\nReturn information about the given bltg address.\n"

            "\nArguments:\n"
            "1. \"bltgaddress\"     (string, required) The bltg address to validate\n"

            "\nResult:\n"
            "{\n"
            "  \"isvalid\" : true|false,         (boolean) If the address is valid or not. If not, this is the only property returned.\n"
            "  \"type\" : \"xxxx\",              (string) \"standard\" or \"sapling\"\n"
            "  \"address\" : \"bltgaddress\",    (string) The bltg address validated\n"
            "  \"scriptPubKey\" : \"hex\",       (string) The hex encoded scriptPubKey generated by the address -only if is standard address-\n"
            "  \"ismine\" : true|false,          (boolean) If the address is yours or not\n"
            "  \"isstaking\" : true|false,       (boolean) If the address is a staking address for BLTG cold staking -only if is standard address-\n"
...
#ifdef ENABLE_WALLET
        // Case 1: BLTG address and we have full public key:
...
UniValue createmultisig(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() < 2 || request.params.size() > 2)
        throw std::runtime_error(
            ...
            "2. \"keys\"       (string, required) A json array of keys which are bltg addresses or hex-encoded public keys\n"
            "     [\n"
            "       \"key\"    (string) bltg address or hex-encoded public key\n"
            "       ,...\n"
            "     ]\n"
...
UniValue verifymessage(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() != 3)
        throw std::runtime_error(
            "verifymessage \"bltgaddress\" \"signature\" \"message\"\n"
            "\nVerify a signed message\n"

            "\nArguments:\n"
            "1. \"bltgaddress\"  (string, required) The bltg address to use for the signature.\n"

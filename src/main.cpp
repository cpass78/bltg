...
#if defined(NDEBUG)
#error "BLTG cannot be compiled without assertions."
#endif
...
/** Fees smaller than this (in ubltg) are considered zero fee (for relaying, mining and transaction creation)
                              ...
                              std::vector<COutPoint>& coins_to_uncache)
{
...

    {
        ...
        if (hasZcSpendInputs) {
            ...
            // Reject legacy zBLTG mints
            if (!Params().IsRegTestNet() && tx.HasZerocoinMintOutputs())
                return state.Invalid(error("%s : tried to include zBLTG mint output in tx %s",
...
int64_t GetBlockValue(int nHeight)
{
    ...
    if (nHeight == 0 ) {
        ...
        nSubsidy = 3000000 * COIN;
    } else if (nHeight <= 20200 && nHeight >= 11) { // ~1 week for grace period
        nSubsidy = 1 * COIN;
...
DisconnectResult DisconnectBlock(CBlock& block, CBlockIndex* pindex, CCoinsViewCache& view)
{
    ...
    //Track zBLTG money supply
    if (!UpdateZBLTGSupplyDisconnect(block, pindex)) {
        error("%s: Failed to calculate new zBLTG supply", __func__);
        return DISCONNECT_FAILED;
    }
...
{
    util::ThreadRename("bltg-scriptch");
...
bool ConnectBlock(const CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, bool fJustCheck, bool fAlreadyChecked)
{
    ...
    // Update zBLTG money supply map
    if (!UpdateZBLTGSupplyConnect(block, pindex, fJustCheck)) {
        return state.DoS(100, error("%s: Failed to calculate new zBLTG supply for block=%s height=%d", __func__,
                                    block.GetHash().GetHex(), pindex->nHeight), REJECT_INVALID);
    }

    // A one-time event where the zBLTG supply was off (due to serial duplication off-chain on main net)
    if (Params().NetworkID() == CBaseChainParams::MAIN && pindex->nHeight == consensus.height_last_ZC_WrappedSerials + 1
        ...
        RecalculateBLTGSupply(consensus.vUpgrades[Consensus::UPGRADE_ZC].nActivationHeight, false);
    }
...
bool CheckBlock(const CBlock& block, CValidationState& state, bool fCheckPOW, bool fCheckMerkleRoot, bool fCheckSig)
{
    ...
    for (const CTransaction& tx : block.vtx) {
        ...
        // double check that there are no double spent zBLTG spends in this block
        if (tx.HasZerocoinSpendInputs()) {
            for (const CTxIn& txIn : tx.vin) {
                bool isPublicSpend = txIn.IsZerocoinPublicSpend();
                if (txIn.IsZerocoinSpend() || isPublicSpend) {
                    ...
                    if (std::count(vBlockSerials.begin(), vBlockSerials.end(), spend.getCoinSerialNumber()))
                        return state.DoS(100, error("%s : Double spending of zBLTG serial %s in block\n Block: %s",
...
bool CheckWork(const CBlock block, CBlockIndex* const pindexPrev)
{
    ...
    if (block.nBits != nBitsRequired){
        ...
        const Consensus::Params& consensus = Params().GetConsensus();
        if ((block.nTime == (uint32_t) consensus.nBltgBadBlockTime) &&
                (block.nBits == (uint32_t) consensus.nBltgBadBlockBits)) {
...
bool AcceptBlock(const CBlock& block, CValidationState& state, CBlockIndex** ppindex, CDiskBlockPos* dbp, bool fAlreadyCheckedBlock)
{
    ...
    if (isPoS) {
        ...
        // Inputs
        std::vector<CTxIn> bltgInputs;
        std::vector<CTxIn> zBLTGInputs;
        ...
        const bool hasZBLTGInputs = !zBLTGInputs.empty();
        ...
        for (const CTransaction& tx : block.vtx) {
            for (const CTxIn& in: tx.vin) {
                ...
                if(hasBLTGInputs) {
                    ...
                    for (const CTxIn& bltgIn : bltgInputs)
                        if(bltgIn.prevout == in.prevout)
        ...
        if (isBlockFromFork) {
            ...
            // Now that this loop if completed. Check if we have zBLTG inputs.
            if(hasZBLTGInputs) {
                for (const CTxIn& zBltgInput : zBLTGInputs) {
                    libzerocoin::CoinSpend spend = TxInToZerocoinSpend(zBltgInput);
                    ...
                    if (!ContextualCheckZerocoinSpendNoSerialCheck(stakeTxIn, &spend, pindex->nHeight, UINT256_ZERO))
                        return state.DoS(100,error("%s: forked chain ContextualCheckZerocoinSpend failed for tx %s", __func__,
                                                   stakeTxIn.GetHash().GetHex()), REJECT_INVALID, "bad-txns-invalid-zbltg");
...
bool static ProcessMessage(CNode* pfrom, std::string strCommand, CDataStream& vRecv, int64_t nTimeReceived, CConnman& connman, std::atomic<bool>& interruptMsgProc)
{
    ...
    if (strCommand == NetMsgType::VERSION) {
        ...
        // BLTG: We use certain sporks during IBD, so check to see if they are
...
int ActiveProtocol()
{
    ...
    // SPORK_15 is used for 70922 (v3.0+), commented out now.

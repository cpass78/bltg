...
void OptionsModel::setDisplayDefaultOptions(QSettings& settings, bool reset)
{
    ...
    if (settings.contains("nAnonymizeBltgAmount") || reset)
        gArgs.SoftSetArg("-anonymizebltgamount", settings.value("nAnonymizeBltgAmount").toString().toStdString());
...
void OptionsModel::Reset()
{
    ...
    settings.clear();
    resetSettings = true; // Needed in bltg.cpp during shotdown to also remove the window positions

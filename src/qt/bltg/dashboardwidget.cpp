...
#include "qt/bltg/dashboardwidget.h"
#include "qt/bltg/forms/ui_dashboardwidget.h"
#include "qt/bltg/sendconfirmdialog.h"
#include "qt/bltg/txrow.h"
...
DashboardWidget::DashboardWidget(BLTGGUI* parent) :
    PWidget(parent),
    ui(new Ui::DashboardWidget)
{
    ...
    setCssProperty(ui->labelSquareBltg, "square-chart-bltg");
    setCssProperty(ui->labelSquarezBltg, "square-chart-zbltg");
    setCssProperty(ui->labelBltg, "text-chart-bltg");
    setCssProperty(ui->labelZbltg, "text-chart-zbltg");
    ...
    setCssProperty(ui->labelAmountBltg, "text-stake-bltg-disable");
    setCssProperty(ui->labelAmountZbltg, "text-stake-zbltg-disable");
...
bool hasCharts = false;
    ...
    if (window)
        connect(window, &BLTGGUI::windowResizeEvent, this, &DashboardWidget::windowResizeEvent);
#endif
...
void DashboardWidget::loadWalletModel()
{
    if (walletModel && walletModel->getOptionsModel()) {
        ...
        connect(ui->pushImgEmpty, &QPushButton::clicked, window, &BLTGGUI::openFAQ);
        connect(ui->btnHowTo, &QPushButton::clicked, window, &BLTGGUI::openFAQ);
    ...
    // update the display unit, to not use the default ("BLTG")
    updateDisplayUnit();
}
...
const QMap<int, std::pair<qint64, qint64>> DashboardWidget::getAmountBy()
{
    ...
    for (int i = 0; i < size; ++i) {
        ...
        bool isBltg = modelIndex.data(TransactionTableModel::TypeRole).toInt() != TransactionRecord::StakeZBLTG;
...
bool DashboardWidget::loadChartData(bool withMonthNames)
{
    ...
    chartData->amountsByCache = getAmountBy(); // pair BLTG, zBLTG
    ...
    for (int j = range.first; j < range.second; j++) {
        ...
        if (chartData->amountsByCache.contains(num)) {
            ...
            bltg = (pair.first != 0) ? pair.first / 100000000 : 0;
            zbltg = (pair.second != 0) ? pair.second / 100000000 : 0;
            chartData->totalBltg += pair.first;
            chartData->totalZbltg += pair.second;
        }

        chartData->xLabels << ((withMonthNames) ? monthsNames[num - 1] : QString::number(num));

        chartData->valuesBltg.append(bltg);
        chartData->valueszBltg.append(zbltg);

        int max = std::max(bltg, zbltg);
...
void DashboardWidget::onChartRefreshed()
{
    ...
    set0->append(chartData->valuesBltg);
    set1->append(chartData->valueszBltg);

    // Total
    nDisplayUnit = walletModel->getOptionsModel()->getDisplayUnit();
    if (chartData->totalBltg > 0 || chartData->totalZbltg > 0) {
        setCssProperty(ui->labelAmountBltg, "text-stake-bltg");
        setCssProperty(ui->labelAmountZbltg, "text-stake-zbltg");
    } else {
        setCssProperty(ui->labelAmountBltg, "text-stake-bltg-disable");
        setCssProperty(ui->labelAmountZbltg, "text-stake-zbltg-disable");
    }
    forceUpdateStyle({ui->labelAmountBltg, ui->labelAmountZbltg});
    ui->labelAmountBltg->setText(GUIUtil::formatBalance(chartData->totalBltg, nDisplayUnit));
    ui->labelAmountZbltg->setText(GUIUtil::formatBalance(chartData->totalZbltg, nDisplayUnit, true));

...
BLTGGUI::BLTGGUI(const NetworkStyle* networkStyle, QWidget* parent) :
        QMainWindow(parent),
        clientModel(0){
...
void BLTGGUI::connectActions()
{
    ...
    connect(topBar, &TopBar::showHide, this, &BLTGGUI::showHide);
    connect(topBar, &TopBar::themeChanged, this, &BLTGGUI::changeTheme);
    connect(topBar, &TopBar::onShowHideColdStakingChanged, navMenu, &NavMenuWidget::onShowHideColdStakingChanged);
    connect(settingsWidget, &SettingsWidget::showHide, this, &BLTGGUI::showHide);
...
#ifndef Q_OS_MAC
void BLTGGUI::trayIconActivated(QSystemTrayIcon::ActivationReason reason)
{
...
void BLTGGUI::closeEvent(QCloseEvent* event)
{
#ifndef Q_OS_MAC // Ignored on Mac
...
static bool ThreadSafeMessageBox(BLTGGUI* gui, const std::string& message, const std::string& caption, unsigned int style)
{

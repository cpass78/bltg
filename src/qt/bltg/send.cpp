...
#include "qt/bltg/forms/ui_send.h"
#include "qt/bltg/addnewcontactdialog.h"
#include "qt/bltg/qtutils.h"
#include "qt/bltg/sendchangeaddressdialog.h"
#include "qt/bltg/optionbutton.h"
#include "qt/bltg/sendconfirmdialog.h"
#include "qt/bltg/guitransactionsutils.h"
#include "qt/bltg/loadingdialog.h"
...
SendWidget::SendWidget(BLTGGUI* parent) :
    PWidget(parent),
    ui(new Ui::send),
    coinIcon(new QPushButton())
{
    ...
    setCssProperty(coinIcon, "coin-icon-bltg");
    ...
    // Connect
    connect(ui->pushLeft, &QPushButton::clicked, [this](){onBLTGSelected(true);});
    connect(ui->pushRight,  &QPushButton::clicked, [this](){onBLTGSelected(false);});
...
void SendWidget::showHideCheckBoxDelegations()
{
    ...
    // coincontrol is not selected, and we are trying to spend transparent BLTGs.
...
void SendWidget::onShieldCoinsClicked()
{
    ...
    if (walletModel && availableBalance > 0) {
        ...
        ProcessSend(recipients, true, [this](QList<SendCoinsRecipient>& recipients) {
            ...
            if (!res.result) {
                inform(tr("Error generating address to shield BLTGs"));
                return false;
            }
        ...
        inform(tr("You don't have any transparent BLTGs to shield."));
    }
}
...
void SendWidget::onBLTGSelected(bool _isTransparent)
{
...
void SendWidget::onContactMultiClicked()
{
    if (focusedEntry) {
        ...
        auto bltgAdd = Standard::DecodeDestination(address.toStdString(), isStakingAddr);

        if (!Standard::IsValidDestination(bltgAdd) || isStakingAddr) {
            inform(tr("Invalid address"));
            return;
        }

        if (walletModel->isMine(bltgAdd)) {
        ...
        if (dialog->res) {
            ...
            if (walletModel->updateAddressBookLabels(bltgAdd, dialog->getLabel().toStdString(),
                    AddressBook::AddressBookPurpose::SEND)) {

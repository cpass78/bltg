...
#ifndef BLTG_STAKEINPUT_H
#define BLTG_STAKEINPUT_H
...
public:
    ...
    virtual bool IsZBLTG() const = 0;
    virtual CDataStream GetUniqueness() const = 0;
    virtual bool ContextCheck(int nHeight, uint32_t nTime) = 0;
};


class CBltgStake : public CStakeInput
{
private:
...
public:
    CBltgStake(const CTxOut& _from, const COutPoint& _outPointFrom, const CBlockIndex* _pindexFrom) :
            CStakeInput(_pindexFrom), outputFrom(_from), outpointFrom(_outPointFrom) {}

    static CBltgStake* NewBltgStake(const CTxIn& txin);
    ...
    bool IsZBLTG() const override { return false; }
    bool ContextCheck(int nHeight, uint32_t nTime) override;
};


#endif //BLTG_STAKEINPUT_H

...
public:
    ...
    bool WriteZBLTGSeed(const uint256& hashSeed, const std::vector<unsigned char>& seed);
    bool ReadZBLTGSeed(const uint256& hashSeed, std::vector<unsigned char>& seed);
    bool ReadZBLTGSeed_deprecated(uint256& seed);
    bool EraseZBLTGSeed();
    bool EraseZBLTGSeed_deprecated();

    bool WriteZBLTGCount(const uint32_t& nCount);
    bool ReadZBLTGCount(uint32_t& nCount);

...
/**
 * Fees smaller than this (in ubltg) are considered zero fee (for transaction creation)
...
int CWallet::ScanForWalletTransactions(CBlockIndex* pindexStart, bool fUpdate, bool fromStartup)
{
    ...
    bool fCheckZBLTG = GetBoolArg("-zapwallettxes", false);
    if (fCheckZBLTG)
        zbltgTracker->Init();
    ...
    CBlockIndex* pindex = pindexStart;
    {
        ...
        while (pindex) {
            ...
            // Will try to rescan it if zBLTG upgrade is active.
            doZBltgRescan(pindex, block, setAddedToWallet, consensus, fCheckZBLTG);
...
bool CWallet::GetMasternodeVinAndKeys(CTxIn& txinRet, CPubKey& pubKeyRet, CKey& keyRet, std::string strTxHash, std::string strOutputIndex, std::string& strError)
{
    ...
    if (txOut.nValue != 10000 * COIN) {
        strError = "Invalid collateral tx value, must be 10,000 BLTG";
        ...
        LOCK2(cs_main, cs_wallet);
        {
            ...
            while (true) {
                ...
                if (nChange > 0) {
                    ...
                    // change transaction isn't always pay-to-bltg-address
...
std::string CWallet::GetWalletHelpString(bool showDebug)
{
    ...
    strUsage += HelpMessageOpt("-minstakesplit=<amt>", strprintf(_("Minimum positive amount (in BLTG) allowed by GUI and RPC for the stake split threshold (default: %s)"), FormatMoney(DEFAULT_MIN_STAKE_SPLIT_THRESHOLD)));
...
CWallet* CWallet::CreateWalletFromFile(const std::string walletFile)
{
    ...
    if (nLoadWalletRet != DB_LOAD_OK) {
        if (nLoadWalletRet == DB_CORRUPT) {
            ...
            UIError(strprintf(_("Error loading %s: Wallet requires newer version of BLTG Core"), walletFile));
            return nullptr;
        } else if (nLoadWalletRet == DB_NEED_REWRITE) {
            UIError(_("Wallet needed to be rewritten: restart BLTG Core to complete"));
            return nullptr;
        } else {
    ...
    if (GetBoolArg("-upgradewallet", fFirstRun && !fLegacyWallet)) {
        if (prev_version <= FEATURE_PRE_BLTG && walletInstance->IsLocked()) {
    ...
    if (fFirstRun) {
        if (!fLegacyWallet) {
            ...
            walletInstance->SetMaxVersion(FEATURE_PRE_BLTG);
        }
    ...
    CzBLTGWallet* zwalletInstance = new CzBLTGWallet(walletInstance);
    ...
    if (!zwalletInstance->GetMasterSeed().IsNull()) {
        //Inititalize zBLTGWallet
        uiInterface.InitMessage(_("Syncing zBLTG wallet..."));

        //Load zerocoin mint hashes to memory
        walletInstance->zbltgTracker->Init();

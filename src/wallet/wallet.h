...
enum WalletFeature {
    ...
    FEATURE_PRE_BLTG = 61000, // inherited version..
...
// Possible states for zBLTG send
enum ZerocoinSpendStatus {
    ZBLTG_SPEND_OKAY = 0,                            // No error
    ZBLTG_SPEND_ERROR = 1,                           // Unspecified class of errors, more details are (hopefully) in the returning text
    ZBLTG_WALLET_LOCKED = 2,                         // Wallet was locked
    ZBLTG_COMMIT_FAILED = 3,                         // Commit failed, reset status
    ZBLTG_ERASE_SPENDS_FAILED = 4,                   // Erasing spends during reset failed
    ZBLTG_ERASE_NEW_MINTS_FAILED = 5,                // Erasing new mints during reset failed
    ZBLTG_TRX_FUNDS_PROBLEMS = 6,                    // Everything related to available funds
    ZBLTG_TRX_CREATE = 7,                            // Everything related to create the transaction
    ZBLTG_TRX_CHANGE = 8,                            // Everything related to transaction change
    ZBLTG_TXMINT_GENERAL = 9,                        // General errors in MintsToInputVectorPublicSpend
    ZBLTG_INVALID_COIN = 10,                         // Selected mint coin is not valid
    ZBLTG_FAILED_ACCUMULATOR_INITIALIZATION = 11,    // Failed to initialize witness
    ZBLTG_INVALID_WITNESS = 12,                      // Spend coin transaction did not verify
    ZBLTG_BAD_SERIALIZATION = 13,                    // Transaction verification failed
    ZBLTG_SPENT_USED_ZBLTG = 14,                      // Coin has already been spend
    ZBLTG_TX_TOO_LARGE = 15,                         // The transaction is larger than the max tx size
    ZBLTG_SPEND_V1_SEC_LEVEL                         // Spend is V1 and security level is not set to 100
};
...
public:
    ...
    /// Get 12000 BLTG output and keys which can be used for the Masternode
    ...
    void doZBltgRescan(const CBlockIndex* pindex, const CBlock& block, std::set<uint256>& setAddedToWallet, const Consensus::Params& consensus, bool fCheckZBLTG);
    ...
    bool CreateZBLTGOutPut(libzerocoin::CoinDenomination denomination, CTxOut& outMint, CDeterministicMint& dMint);
    ...
    // zBLTG wallet
    std::unique_ptr<CzBLTGTracker> zbltgTracker{nullptr};
    void setZWallet(CzBLTGWallet* zwallet);
    CzBLTGWallet* getZWallet();

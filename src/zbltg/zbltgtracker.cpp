...
CzBLTGTracker::CzBLTGTracker(CWallet* parent)
{
    this->wallet = parent;
...
CzBLTGTracker::~CzBLTGTracker()
{
    mapSerialHashes.clear();
    mapPendingSpends.clear();
}

void CzBLTGTracker::Init()
{
...
bool CzBLTGTracker::Archive(CMintMeta& meta)
{
...
bool CzBLTGTracker::UnArchive(const uint256& hashPubcoin, bool isDeterministic)
{
    CWalletDB walletdb(wallet->strWalletFile);
    if (isDeterministic) {
        CDeterministicMint dMint;
...
CMintMeta CzBLTGTracker::Get(const uint256 &hashSerial)
{
...
CMintMeta CzBLTGTracker::GetMetaFromPubcoin(const uint256& hashPubcoin)
{
...
bool CzBLTGTracker::GetMetaFromStakeHash(const uint256& hashStake, CMintMeta& meta) const
{
    for (auto& it : mapSerialHashes) {
...
CAmount CzBLTGTracker::GetBalance(bool fConfirmedOnly, bool fUnconfirmedOnly) const
{
    CAmount nTotal = 0;
...
CAmount CzBLTGTracker::GetUnconfirmedBalance() const
{
    return GetBalance(false, true);
}

std::vector<CMintMeta> CzBLTGTracker::GetMints(bool fConfirmedOnly) const
{
...
bool CzBLTGTracker::HasMintTx(const uint256& txid)
{
    for (auto it : mapSerialHashes) {
...
bool CzBLTGTracker::HasPubcoin(const CBigNum &bnValue) const
{
...
bool CzBLTGTracker::HasPubcoinHash(const uint256& hashPubcoin) const
{
    for (auto it : mapSerialHashes) {
        CMintMeta meta = it.second;
        if (meta.hashPubcoin == hashPubcoin)
...
bool CzBLTGTracker::HasSerial(const CBigNum& bnSerial) const
{
    uint256 hash = GetSerialHash(bnSerial);
    return HasSerialHash(hash);
}

bool CzBLTGTracker::HasSerialHash(const uint256& hashSerial) const
{
    auto it = mapSerialHashes.find(hashSerial);
    return it != mapSerialHashes.end();
}

bool CzBLTGTracker::UpdateZerocoinMint(const CZerocoinMint& mint)
{
...
bool CzBLTGTracker::UpdateState(const CMintMeta& meta)
{
    CWalletDB walletdb(wallet->strWalletFile);
...
void CzBLTGTracker::Add(const CDeterministicMint& dMint, bool isNew, bool isArchived, CzBLTGWallet* zBLTGWallet)
{
    bool iszBLTGWalletInitialized = (nullptr != zBLTGWallet);
    CMintMeta meta;
    ...
    if (!iszBLTGWalletInitialized)
        zBLTGWallet = new CzBLTGWallet(wallet);
    meta.isSeedCorrect = zBLTGWallet->CheckSeed(dMint);
    if (!iszBLTGWalletInitialized)
        delete zBLTGWallet;
    mapSerialHashes[meta.hashSerial] = meta;
...
void CzBLTGTracker::Add(const CZerocoinMint& mint, bool isNew, bool isArchived)
{
    CMintMeta meta;
...
void CzBLTGTracker::SetPubcoinUsed(const uint256& hashPubcoin, const uint256& txid)
{
...
void CzBLTGTracker::SetPubcoinNotUsed(const uint256& hashPubcoin)
{
    if (!HasPubcoinHash(hashPubcoin))
        return;
    CMintMeta meta = GetMetaFromPubcoin(hashPubcoin);
...
void CzBLTGTracker::RemovePending(const uint256& txid)
{
    uint256 hashSerial;
...
bool CzBLTGTracker::UpdateStatusInternal(const std::set<uint256>& setMempool, CMintMeta& mint)
{
...
std::set<CMintMeta> CzBLTGTracker::ListMints(bool fUnusedOnly, bool fMatureOnly, bool fUpdateStatus, bool fWrongSeed, bool fExcludeV1)
{
    CWalletDB walletdb(wallet->strWalletFile);
    if (fUpdateStatus) {
        ...
        LogPrint(BCLog::LEGACYZC, "%s: added %d dzbltg from DB\n", __func__, listDeterministicDB.size());
    }

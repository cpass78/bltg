...
CzBLTGWallet::CzBLTGWallet(CWallet* parent)
{
    this->wallet = parent;
    ...
    //Check for old db version of storing zbltg seed
    if (fFirstRun) {
        uint256 seed;
        if (walletdb.ReadZBLTGSeed_deprecated(seed)) {
            //Update to new format, erase old
            seedMaster = seed;
            hashSeed = Hash(seed.begin(), seed.end());
            if (wallet->AddDeterministicSeed(seed)) {
                if (walletdb.EraseZBLTGSeed_deprecated()) {
                    LogPrintf("%s: Updated zBLTG seed databasing\n", __func__);
                    fFirstRun = false;
                } else {
                    LogPrintf("%s: failed to remove old zbltg seed\n", __func__);
                }
            }
        }
    }
    ...
    if (fRegtest && fFirstRun) {
        ...
        seedMaster = seed;
        LogPrintf("%s: first run of zbltg wallet detected, new seed generated. Seedhash=%s\n", __func__, Hash(seed.begin(), seed.end()).GetHex());
...
bool CzBLTGWallet::SetMasterSeed(const uint256& seedMaster, bool fResetCount)
{
    ...
    if (fResetCount)
        walletdb.WriteZBLTGCount(nCountLastUsed);
    else if (!walletdb.ReadZBLTGCount(nCountLastUsed))
        nCountLastUsed = 0;
...
void CzBLTGWallet::Lock()
{
    seedMaster.SetNull();
}

void CzBLTGWallet::AddToMintPool(const std::pair<uint256, uint32_t>& pMint, bool fVerbose)
{
    mintPool.Add(pMint, fVerbose);
}

//Add the next 20 mints to the mint pool
void CzBLTGWallet::GenerateMintPool(uint32_t nCountStart, uint32_t nCountEnd)
{
    ...
    for (uint32_t i = n; i < nStop; ++i) {
        ...
        CKey key;
        SeedToZBLTG(seedZerocoin, bnValue, bnSerial, bnRandomness, key);
...
bool CzBLTGWallet::LoadMintPoolFromDB()
{
...
void CzBLTGWallet::RemoveMintsFromPool(const std::vector<uint256>& vPubcoinHashes)
{
    for (const uint256& hash : vPubcoinHashes)
        mintPool.Remove(hash);
}

void CzBLTGWallet::GetState(int& nCount, int& nLastGenerated)
{
...
void CzBLTGWallet::SyncWithChain(bool fGenerateMintPool)
{
    ...
    while (found) {
        ...
        for (std::pair<uint256, uint32_t> pMint : listMints) {
            ...
            if (wallet->zbltgTracker->HasPubcoinHash(pMint.first)) {
...
bool CzBLTGWallet::SetMintSeen(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const libzerocoin::CoinDenomination& denom)
{
    ...
    CBigNum bnRandomness;
    CKey key;
    SeedToZBLTG(seedZerocoin, bnValueGen, bnSerial, bnRandomness, key);
    ...
    // Add to zbltgTracker which also adds to database
    wallet->zbltgTracker->Add(dMint, true);
...
void CzBLTGWallet::SeedToZBLTG(const uint512& seedZerocoin, CBigNum& bnValue, CBigNum& bnSerial, CBigNum& bnRandomness, CKey& key)
{
    libzerocoin::ZerocoinParams* params = Params().GetConsensus().Zerocoin_Params(false);
...
uint512 CzBLTGWallet::GetZerocoinSeed(uint32_t n)
{
...
void CzBLTGWallet::GenerateDeterministicZBLTG(libzerocoin::CoinDenomination denom, libzerocoin::PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly)
{
...
void CzBLTGWallet::GenerateMint(const uint32_t& nCount, const libzerocoin::CoinDenomination denom, libzerocoin::PrivateCoin& coin, CDeterministicMint& dMint)
{
...
bool CzBLTGWallet::CheckSeed(const CDeterministicMint& dMint)
{
...
bool CzBLTGWallet::RegenerateMint(const CDeterministicMint& dMint, CZerocoinMint& mint)
{
    if (!CheckSeed(dMint)) {
        uint256 hashSeed = Hash(seedMaster.begin(), seedMaster.end());

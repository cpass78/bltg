...
bool CLegacyZBltgStake::InitFromTxIn(const CTxIn& txin)
{
    // Construct the stakeinput object
    if (!txin.IsZerocoinSpend())
        return error("%s: unable to initialize CLegacyZBltgStake from non zc-spend", __func__);
    ...
    *this = CLegacyZBltgStake(spend);

    // Find the pindex with the accumulator checksum
    if (!GetIndexFrom())
        return error("%s : Failed to find the block index for zbltg stake origin", __func__);

    // All good
    return true;
}

CLegacyZBltgStake::CLegacyZBltgStake(const libzerocoin::CoinSpend& spend) : CStakeInput(nullptr)
{
...
const CBlockIndex* CLegacyZBltgStake::GetIndexFrom() const
{
...
CAmount CLegacyZBltgStake::GetValue() const
{
    return denom * COIN;
}

CDataStream CLegacyZBltgStake::GetUniqueness() const
{
...
bool CLegacyZBltgStake::ContextCheck(int nHeight, uint32_t nTime)
{
    const Consensus::Params& consensus = Params().GetConsensus();
    if (!consensus.NetworkUpgradeActive(nHeight, Consensus::UPGRADE_ZC_V2) || nHeight >= consensus.height_last_ZC_AccumCheckpoint)
        return error("%s : zBLTG stake block: height %d outside range", __func__, nHeight);

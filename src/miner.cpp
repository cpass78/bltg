...
{
    // double check that there are no double spent zBLTG spends in this block or tx
    if (tx.HasZerocoinSpendInputs()) {
        ...
        //This zBLTG serial has already been included in the block, do not add this tx.
        if (fDoubleSerial) {
            return false;
        }
    }
...

    {
        ...
        for (CTxMemPool::indexed_transaction_set::iterator mi = mempool.mapTx.begin();
            ...
            for (const CTxIn& txin : tx.vin) {
                ...
                // zBLTG spends can have very large priority, use non-overflowing safe functions
        ...
        while (!vecPriority.empty()) {
            ...
            // zBLTG check to not include duplicated serials in the same block.
...
bool ProcessBlockFound(CBlock* pblock, CWallet& wallet, Optional<CReserveKey>& reservekey)
{
    ...
    if (!ProcessNewBlock(state, nullptr, pblock, nullptr, g_connman.get())) {
        return error("BLTGMiner : ProcessNewBlock, block not accepted");
    }
...
{
    LogPrintf("BLTGMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    util::ThreadRename("bltg-miner");
    const Consensus::Params& consensus = Params().GetConsensus();

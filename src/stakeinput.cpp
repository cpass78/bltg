...
CBltgStake* CBltgStake::NewBltgStake(const CTxIn& txin)
{
    if (txin.IsZerocoinSpend()) {
        error("%s: unable to initialize CBltgStake from zerocoin spend", __func__);
        return nullptr;
    }
...
const CBlockIndex* CBltgStake::GetIndexFrom() const
{
    // Sanity check, pindexFrom is set on the constructor.
    if (!pindexFrom) throw std::runtime_error("CBltgStake: uninitialized pindexFrom");
    return pindexFrom;
}

...
bool CBltgStake::InitFromTxIn(const CTxIn& txin)
{
    if (txin.IsZerocoinSpend())
        return error("%s: unable to initialize CBltgStake from zerocoin spend", __func__);
...
bool CBltgStake::SetPrevout(CTransaction txPrev, unsigned int n)
{
    this->txFrom = txPrev;
    this->nPosition = n;
    return true;
...
bool CBltgStake::GetTxFrom(CTransaction& tx) const
{
    if (txFrom.IsNull())
        return false;
    tx = txFrom;
...
bool CBltgStake::GetTxOutFrom(CTxOut& out) const
{
...
bool CBltgStake::CreateTxIn(CWallet* pwallet, CTxIn& txIn, uint256 hashTxOut)
{
    txIn = CTxIn(txFrom.GetHash(), nPosition);
    return true;
}

CAmount CBltgStake::GetValue() const
{
    return txFrom.vout[nPosition].nValue;
}

bool CBltgStake::CreateTxOuts(CWallet* pwallet, std::vector<CTxOut>& vout, CAmount nTotal, const bool onlyP2PK)
{
...
CDataStream CBltgStake::GetUniqueness() const
{
    //The unique identifier for a BLTG stake is the outpoint
...
CBlockIndex* CBltgStake::GetIndexFrom()
{
    if (pindexFrom)
        return pindexFrom;
...
bool CBltgStake::ContextCheck(int nHeight, uint32_t nTime)
{

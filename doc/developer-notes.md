...
- [Development guidelines](#development-guidelines)
    - [General BLTG Core](#general-bltg-core)
    - [Wallet](#wallet)
    - [General C++](#general-c)
...
BLTG Core uses [Doxygen](http://www.doxygen.nl/) to generate its official documentation.
...
Run with the `-testnet` option to run with "play BLTGs (tBLTG)" on the test network, if you
...
BLTG Core is a multi-threaded application, and deadlocks or other
...
([`valgrind.supp`](https://github.com/Block-Logic-Technology-Group/bltg/blob/master/contrib/valgrind.supp))
...
# A coverage report will now be accessible at `./test_bltg.coverage/index.html`.
```
...
However, in open source software such as BLTG Core, where everyone uses
...
pay attention to for reviewers of BLTG Core code.
...
- For `strprintf`, `LogPrint`, `LogPrintf` formatting characters don't need size specifiers
  ...
  - *Rationale*: BLTG Core uses tinyformat, which is type safe. Leave them out to avoid confusion
...
be sent upstream but bugfixes may also be prudent to PR against BLTG Core so that they can be integrated
...
file descriptors it will cause problems with BLTG's `select()` loop, because
...
- When looking at other's pull requests, it may make sense to add the following section to your `.git/config`
  file:
        ...
        [remote "upstream-pull"]
                ...
                url = git@github.com:Block-Logic-Technology-Group/bltg.git
...
- Try not to overload methods on argument type. E.g. don't make `getblock(true)` and `getblock("hash")`
  ...
  - *Rationale*: This is impossible to use with `bltg-cli`, and can be surprising to users.
...
- Add every non-string RPC argument `(method, idx, name)` to the table `vRPCConvertParams` in `rpc/client.cpp`.
  ...
  - *Rationale*: `bltg-cli` and the GUI debug console use this table to determine how to

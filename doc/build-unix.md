UNIX BUILD NOTES
====================
Some notes on how to build BLTG Core in Unix.

Note
---------------------
Always use absolute paths to configure and compile BLTG Core and the dependencies,
...
This will build bltg-qt as well, if the dependencies are met.
...
memory available when compiling BLTG Core. On systems with less, gcc can be
...
To build BLTG Core without wallet, see [*Disable-wallet mode*](/doc/build-unix.md#disable-wallet-mode)
...
If you want to build bltg-qt, make sure that the required packages for Qt development
...
Notes
-----
The release is built with GCC and then "strip bltgd" to strip the debug
...
* _Non-executable Stack_: If the stack is executable then trivial stack-based buffer overflow exploits are possible if
    vulnerable buffers are found. By default, BLTG Core should be built with a non-executable stack
...
When the intention is to run only a P2P node without a wallet, BLTG Core may be compiled in
